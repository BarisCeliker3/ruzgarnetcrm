                         ...
                         <table class="table table-striped" id="dataTable">
                            <thead style="font-size:15px;background:#ff9108;">
								<tr>
                                    ...
                                    <th style="color:#fff;" scope="col">İşlemler</th>
                                </tr>
                            </thead>
                            <tbody>
                                   ...
                                   @foreach ($internetsetups as $internet )
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{route('admin.internetsetup.edit', $internet->id)}}">
       ...
       <!-- START HAZIRLIK -->
            <div class="card list">
                <div class="card-header">
                    ...
                    <h4 style="font-size:22px;"> 
                    <span style="color:#0000b4;">Tamamlanan veya İptal edilen kurulum listesi</span></h4>
                         ...
                         <table class="table table-striped" id="dataTable2">
                            <thead style="font-size:15px;background:#ff9108;">
								<tr>
                                    ...
                                    <th style="color:#fff;" scope="col">Şifre</th>
                                    <th style="color:#fff;" scope="col">Tarih</th>
                                    <th style="color:#fff;" scope="col">Durum</th>
                                </tr>
                            </thead>
                            <tbody>

                                   @foreach ($internetsetups2 as $internet1 )
                                    <tr>
                                        <th scope="row"><span class="text-danger">{{ $loop->iteration }}</span></th>
                                        <td>{{ $internet1->customer_name }}<br>{{ $internet1->telephone }}</td>
                                        <td>
                                            <span class="d-inline-block" tabindex="0" data-toggle="tooltip" data-html="true"  data-placement="right" title="{{ $internet1->adress }}">
                                                <button class="not-btn btn btn-info badge-pill" style="pointer-events: none;" type="button" disabled>Adres</button>
                                            </span>
                                        </td>
                                        <td>{{ $internet1->bbk_code }}</td>
                                        <td>{{ $internet1->users_name }}</td>
                                        <td>{{ $internet1->users_password }}</td>
                                       
                                        <td>{{ $internet1->start_date }}</td>
                                            @if($internet1->status == 2)
                                            <td style="font-size:16px; font-weight: bold; " class="text-success">KURULDU</td>
                                            @elseif($internet1->status == 3)
            ...
            $("#dataTable").dataTable({
                ...
                initComplete: function () {
                    this.api().columns().every( function () {
                        var column = this;
                        if(column[0][0] != 0 && column[0][0] != 1 && column[0][0] != 2 && column[0][0] != 3  && column[0][0] != 4 && column[0][0] != 5 && column[0][0] != 6 && column[0][0] != 7 && column[0][0] != 8 && column[0][0] != 9 )
                        {
         ...
         $(function() {
            $("#dataTable2").dataTable({
                ...
                initComplete: function () {
                    this.api().columns().every( function () {
                        var column = this;
                        if(column[0][0] != 0 && column[0][0] != 1 && column[0][0] != 2 && column[0][0] != 3  && column[0][0] != 4 && column[0][0] != 5 && column[0][0] != 6)
                        {
                            var select = $('<select class="form-control text-white bg-primary" style="width:100px;"><option value="">TÜMÜ</option></select>')
                            .appendTo( $(column.header()).empty() )
                            .on( 'change', function () {
