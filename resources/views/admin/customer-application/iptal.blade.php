...
@section('title', meta_title('tables.rzgcancel.title'))
        ...
        <div class="col-12">
            <div class="card list">
                <div class="card-header">
                    <h4> <span class="text-white">İptal - İkna / Hizmet No Formu</span></h4>
                            ...
                            <tbody>
                                @foreach ($customer_applications as $index => $customer_application)
                                    <tr data-id="{{ $customer_application->id }}">
                                        ...
                                        <td>
                                         @if($customer_application->searchStatus =='0')
                                           <span class="badge badge-light badge-pill"> 
                                              <span style="font-size:14px;" class="badge badge-pill badge-danger">X</span>
                                          </span>
                                          @elseif($customer_application->searchStatus =='1')
                                            <span class="badge badge-light badge-pill"> 
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('admin.customer.show', $customer_application->customer->id) }}"
                                                target="_blank">
                                                <span  class="referenced-subscription">
                                                    {{ preg_replace('~.*(\d{3})[^\d]{0,7}(\d{3})[^\d]{0,7}(\d{4}).*~', '($1) $2-$3', $customer_application->customer->telephone) }}
                                                </span>
                                            </a>
                                        @else
                                        ...
                                        <td> @foreach(json_decode($customer_application->files, true) as $key => $value)
                                        ...
                                        <td>
                                           <button type="button" class="btn btn-info btn-sm" 
                                            ...
                                            title="{{ $customer_application->description }}">
                                              Not
                                           </button>
                                        ...
                                        <td>
                                            <span title="{{ convert_date($customer_application->created_at, 'large') }}" class="text-dark">
                                                {{ $customer_application->created_at }}
                                            </span>
                                        </td>
                                        <td>
                                            <div class="buttons">
            ...
            $("#dataTable").dataTable({
                ...
                initComplete: function () {
                    this.api().columns().every( function () {
                        var column = this;
                        if(column[0][0] != 0 && column[0][0] != 1 && column[0][0] != 2 && column[0][0] != 3 && column[0][0] != 5 && column[0][0] != 7 && column[0][0] != 8 && column[0][0] != 9 && column[0][0] != 10)
                        {
