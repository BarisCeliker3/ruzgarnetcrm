    ...
    <!-- START Sözleşmenin Bitmesine 15 Gün Kalan -->
            <div class="card list">
                <div class="card-header">
                    ...
                    <span style="color:#0000b4;">Rüzgar </span><span class="text-warning">Çözüm Birimi</span> 
                     <span class="text-dark">(R.Ç.B)</span></h4>
           ...
           <hr style="border-bottom:2px solid #FF0000;">  
                <div class="col-md-12 form-row">
                    <div class="form-group col-md-4">
                      ...
                      <input value="{{$subsupendiki->customer->first_name}} {{$subsupendiki->customer->last_name}}" type="text" class="form-control" disabled>
                    </div>
                    
                    <div class="form-group col-md-4">
                      <label style="color:#304FFE; font-weight: bold;font-size:15px;" for="inputEmail4">Tarife</label>
                      <input value="{{$subsupendiki->service->name}}"  type="text" class="form-control" disabled>
                    </div>
                    <div class="form-group col-md-4">
                      <label style="color:#304FFE; font-weight: bold;font-size:15px;" for="inputEmail4">Telefon</label>
                      <input value="{{ preg_replace('~.*(\d{3})[^\d]{0,7}(\d{3})[^\d]{0,7}(\d{4}).*~', '($1) $2-$3', $subsupendiki->customer->telephone) }}"  type="text" class="form-control" disabled>
                    </div>
                </div> 
                    
             <form method="POST" action="{{relative_route('admin.rcb.edit.post',$subsupendiki->id)}}" class="needs-validation" novalidate>
                 @csrf
                <!-- task1 status1   --> 
             <input value="{{$subsupendiki->id}}" name="subscription_id" type="hidden" class="form-control" >
            ...
            <!-- task1 status1   --> 
                  <div class="col-md-12 form-row">
                    <div class="form-group col-md-12">
                      ...
                      <textarea  name="note" type="text" class="form-control">{{$subsupendiki->note}}</textarea>
                    </div>
                   <div class="form-group col-md-6">
                      <label style="color:#304FFE; font-weight: bold;font-size:15px;" for="inputEmail4">Durum</label>
                      <select name="status"  class="form-control">
                         <option @if($subsupendiki->status==='0') selected @endif value="0">Aranmadı</option>
                         <option @if($subsupendiki->status==='1') selected @endif value="1">Arandı</option>
                      </select>
                   </div>
                   <div class="form-group col-md-6">
                      <label style="color:#304FFE; font-weight: bold;font-size:15px;" for="inputEmail4">Sonuç</label>
                      <select name="result" class="form-control">
                         <option @if($subsupendiki->result==='0') selected @endif value="0">Memnun Değil</option>
                         <option @if($subsupendiki->result==='1') selected @endif value="1">Memnun</option>
                         <option @if($subsupendiki->result==='2') selected @endif value="2">Ulaşılamadı</option>
                      </select>
                   </div>
                  </div><br><br>
           <!-- end task1 status1   --> 
                  <button style="float:right; margin-top:-33px"  type="submit" class="btn btn-success">Güncelle</button>
            </form>
