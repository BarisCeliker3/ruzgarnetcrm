...
@section('title', meta_title('tables.role.edit'))

@section('content')
    <div class="row">
        <div class="col-12">
            <form method="POST" action="{{ relative_route('admin.role.edit.put', $role) }}">
                @method('put')
                <div class="card form">
                    <div class="card-header">
                        <h4>@lang('tables.role.edit') [{{ $role->id }}]</h4>
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" id="inpName" class="form-control" value="{{ $role->name }}">
                        </div>
                                            ...
                                            @foreach ($group as $ability)
                                                <div class="custom-control custom-checkbox">
                                                    <input type="checkbox" class="custom-control-input" name="abilities[]"
                                                        value="{{ $ability->key }}" id="chkAbility{{ $ability->id }}" @if (in_array($ability->key, $roleAbilities)) checked @endif>
...
@push('script')
    ...
    <script src="/assets/admin/vendor/cleave/editons/cleave-phone.tr.js"></script>
@endpush
