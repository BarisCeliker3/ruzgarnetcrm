...
class CheckCommitmentuc extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ruzgarnet:checkCommitmentuc';
...
//
    ...
    // TAAHHÜT SÜRESİ 5 AYDAN BÜYÜK OLANLARA 30 GÜN SMS GÖNDER
            $subscriptions = Subscription::where('status', 1)->where('commitment','>=', '5')->whereRaw('DATEDIFF(`end_date`, NOW()) = 30')->get();
          foreach ($subscriptions as $subscription) {
            /* 30 Gün SMS GÖNDERME
              $message_text = [
                  $subscription->customer->telephone,
              ...
              SentMessage::create([
                  ...
                  'delivery_date' => date('Y-m-d 12:25')
              ]);
