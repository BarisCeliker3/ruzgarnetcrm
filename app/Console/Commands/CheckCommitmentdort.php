...
class CheckCommitmentdort extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ruzgarnet:checkCommitmentdort';
    ...
    public function handle()
    {
        try {
         ...
         //   $message_45 = Message::find(40)->message;
            ...
            $message_15_2 = Message::find(81)->message;
...
//              }catch(Exception $e)
//              {
//                  Telegram::send('Test', 'CheckCommitmentdort Command - ' . $e->getMessage());
//              }
//          }
//
             
        // TAAHHÜT SÜRESİ 4 AY'DAN KÜÇÜK OLAN ABONELER İÇİN 15 GÜN SMS'Sİ 
             $subscriptions = Subscription::where('status', 1)->where('commitment','<=', '4')->whereRaw('DATEDIFF(`end_date`, NOW()) = 15')->get();
            foreach ($subscriptions as $subscription) {
              $new_price = $subscription->price;
              if ($renewal = $subscription->renewal()) {
                  $new_price = $renewal->new_price;
			  ...
			  $message_text = [
                  $subscription->customer->telephone,
                  $message_formatter->generate($message_15_2, [
                      'ad_soyad' => $subscription->customer->full_name,
                      'tarife' => $subscription->service->name,
                      'tutar' => $new_price
                    ])
              ...
              SentMessage::create([
                  ...
                  'delivery_date' => date('Y-m-d 15:20')
              ]);
...
*/
          ...
          // TAAHHÜT SÜRESİ 5 AY'DAN BÜYÜK OLAN ABONELER İÇİN 15 GÜN SMS'Sİ 
             $subscriptions = Subscription::where('status', 1)->where('commitment','>=', '5')->whereRaw('DATEDIFF(`end_date`, NOW()) = 15')->get();
            foreach ($subscriptions as $subscription) {
              $new_price = $subscription->price;
              if ($renewal = $subscription->renewal()) {
                  $new_price = $renewal->new_price;
              ...
              SentMessage::create([
                  'phone' => $message_text[0],
                  'message' => $message_text[1],
                  'delivery_date' => date('Y-m-d 15:25')
              ]);
