...
class Customer extends Model
{
    ...
    public function getStaffAttribute()
    {
        if (!$this->staffs()->count()) {
            ...
            $staff_id = DB::table('staff')->select('id')->whereRaw('id NOT IN (SELECT staff_id FROM customer_staff)')->whereRaw("id IN (SELECT staff_id FROM users WHERE role_id = 4)")->limit(1)->first()->id ?? null;
        ...
        try {
            ...
            SentMessage::insert([
                [
                    ...
                    'message_id' => 49,
                    'delivery_date' => date('Y-m-d H:i:s', strtotime('+30 minute'))
                ]
            ]);
