...
class Subscription extends Model
{
     ...
     * @return \App\Models\SubscriptionRenewal|null
     */
    public function renewaled()
    {
        if ($this->renewals()->count()) {
            return $this->renewals()->where('status', 1)->orderByDesc('id')->first();
        }
        return null;
    }
...
/**
     ...
     * @return \App\Models\Payment
     */
    public function lastUnpaidPayment()
    {

        $last_unpaid = Payment::where('subscription_id', $this->id)
        ->where('date','<=', date('Y-m-15'))
        ->whereNull('paid_at')
        ->orderBy('date', 'ASC')
        ->first();
    ...
    public function nextMonthPayment()
    {
        ...
        $last_paid = Payment::where('subscription_id', $this->id)
        ->whereNull('paid_at')
        ->orderBy('date', 'ASC')
        ->first();
        $date = Carbon::parse( $last_paid->date);
        ...
        if (!$payment) {
            return Payment::create([
                'subscription_id' => $this->id,
                'price' => $this->price,
                'date' => $date->addMonth(1)->format('Y-m-15')
            ]);
        }
        return $payment;
    }
    ...
    public function approve_subscription()
    {
        ...
        try {
            ...
            // Set the first price after 28
            // 28'ten sonra aboneliği eklenirse ilk ay ücretini yarıya düş
